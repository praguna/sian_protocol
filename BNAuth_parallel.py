...
class BNAuth(object):
    ...
    def __init__(self, X, M, party_type : Party, socket: list = None, direct_index = False ,error_rate = 0.25) -> None:
        '''
        arg X -> template
        ...
        self.T = []
        self.visited_mask_pos = np.zeros_like(self.M)
        self.count  = 0
        self.p = None
        assert self.m > 0
    

    def send_to_peer(self, data, idx = 0):
        '''
        sends it to the peer socket
        '''
        # if self.p is not None: print(idx)
        self.socket[idx].sendall(bytes(data, encoding="utf-8"))
        # print(f'sent : {data}')

    
    def recieve_from_peer(self, num_bytes = 126000, idx = 0):
        '''
        ...
        msg = self.socket[idx].recv(num_bytes).decode('utf-8')
    ...
    def get_masked_bit_quad(self):
        ...
        def P1():
            ...
            while not is_expected_noise:
                ...
                is_expected_noise  = self.recieve_has_expected_noise(pos, xor_on_numpy1D(self.M[pos]))
            return self.M[pos] # implementing only for p1
        ...
        def P2():
            ...
            while not is_expected_noise:
                ...
                if xor_on_numpy1D(self.M[data['pos']]) != data['masked_xor'] and np.sum(self.visited_mask_pos[data['pos']]) < 2: is_expected_noise = True
            ...
            return self.M[data['pos']] # implementing only for p1
    ...
    def send_xors_over(self, x1_xor_a1, y1_xor_b1, idx = 0):
        '''
        sends x1 xor a1 and y1 xor b1 to another party
        '''
        self.send_to_peer(json.dumps({'xors' : (bool(x1_xor_a1), bool(y1_xor_b1))}), idx)

    def recieve_xors(self, idx = 0):
        '''
        recieve xors from the other party i.e :  x2 xor a2 and y2 xor b2
        '''
        a = self.recieve_from_peer(idx = idx)
        return a['xors']
    ...
    def fetch_octect_bulk(self, batch_size = 128, i = 0):
        '''
        gets octects for each party
        '''
        def P1():
            idx = np.random.choice(list(self.selected_octect_index), batch_size)
            self.send_to_peer(json.dumps({'idx' : serialize_nd_array(idx)}), i)
            return idx
        def P2():
            idx = self.recieve_from_peer(idx = i)['idx']
            return idx
        idx = P1() if self.party_type == Party.P1 else P2()
        return idx
    ...
    def perform_computation_phase_v2(self, octect, w, v, noise = False, idx = 0):
        '''
        ...
        self.send_xors_over(x1_xor_a1, y1_xor_b1, idx)
        x2_xor_a2, y2_xor_b2  = self.recieve_xors(idx)
        XA = x1_xor_a1 ^ x2_xor_a2
        YB = y1_xor_b1 ^ y2_xor_b2
        Z =  (XA & YB) ^ (w & YB) ^ (v & XA) ^ c1  if self.party_type == Party.P1 else (w & YB) ^ (v & XA) ^ c1 
        return Z

    def perform_computation_phase_v3(self, octect, w, v, noise = False, idx = 0):
        '''
        optimized without util function calls
        '''
        a1, b1, c1 = octect[2] ^ octect[3] , octect[1] ^ octect[3] , octect[3] if not noise else  octect[0] ^ octect[1] ^ octect[2]
        x1_xor_a1, y1_xor_b1 = a1^w , b1^v
        self.socket[idx].sendall(struct.pack('??',x1_xor_a1, y1_xor_b1))
        s = self.socket[idx].recvmsg(2)[0]
    ...
    def perform_batch_match(self, batch, s, e, noise = False, q = None, idx = 0):
        '''
        secure match on batch
        '''
        s_ = 0
        lt = int(np.log2(batch)) 
        for k in range(s, e, batch):
            W = [self.Z1[k]]
            v =  None
            octets = self.fetch_octect_bulk(batch, idx)
            for i in range(batch-1):
                ...
                while j>=0:  
                    w = W[j]
                    f = w^v
                    v = self.perform_computation_phase_v3(self.octects[octets[i]].ravel(), w, v, noise, idx)
                    W[j] = f
                    j-=1  
                if len(W) <= lt: W.insert(0, v)
            self.send_to_peer(json.dumps({'W2' : serialize_nd_array(W)}), idx)
            W2 = self.recieve_from_peer(idx=idx)['W2']
            s_+=self.calculate_sum(W, W2)
        if q: q.put(s_)
    ...
    def perform_secure_match(self, noise = False):
        ...
        self.Z1 = self.perform_secure_xor(self.X1, self.Y1)

        s, batch = 0, 128
        q = multiprocessing.Queue()
        n = 4
        pool = [threading.Thread(target=self.perform_batch_match, args=[batch, (self.d * i)//n, (self.d * (i+1))//n, noise, q, i]) for i in range(n)]
